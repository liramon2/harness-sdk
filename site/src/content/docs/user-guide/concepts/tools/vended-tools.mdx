...
mcp_router = make_mcp_router(
    name="mcp_router",
    description="Routes agent requests to permitted MCP servers.",
    description_suffix="Use 'files' for filesystem access and 'api' for REST calls.",
    servers={
        "files": {
            "command": "npx",
...
agent = Agent(tools=[mcp_router])
agent(
    "Connect to 'files' with connection_id 'fs', connect to 'api' with "
    "connection_id 'api', list tools on each, then disconnect both."
)
```

...
sourceLinks:
  ...
  - path: strands-py/src/strands/experimental/tools/code_execution/code_execution.py
---
...
| [Code Execution](#code-execution-experimental) | Execute Python code in a sandboxed interpreter with persistent session state | Python |
...
agent("Use code_execution to compute the first 10 Fibonacci numbers and print them.")
```
...
```python
from strands import Agent
from strands.experimental.tools import make_code_execution

code_exec = make_code_execution(
